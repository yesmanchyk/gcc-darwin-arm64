...
{
  S s;
  s.[: ^^S::tfn :](42); // { dg-error "reflection not usable in a splice expression" }
// { dg-message "add .template. to denote a template" "" { target *-*-* } .-1 }
  s.template [: ^^S::tfn :](42);

  constexpr auto r = ^^fortytwo;
  constexpr int i1 = [:r:]<int>; // { dg-error "reflection not usable in a splice expression with template arguments" }
// { dg-message "add .template. to denote a template" "" { target *-*-* } .-1 }
